...
void move(sortinfo *sortinfo, stacks *stacks, int a)
{
    ...
    while(j < 3)
    {
        ...
        printf("\n\n%d == %d \n\n\n", size , sortinfo->mergeSize[sortinfo->len -1]);
        // printf("len : %d\n", sortinfo->len);
        for(int i = 0; i < sortinfo->len; i++)
        {
            printf("triShape : %d ", sortinfo->triShape[i]);
            printf("mergeSize : %d \n\n", sortinfo->mergeSize[i]);
        }
...
void reallowerSort(sortsize *ss, stacks *stacks)
{
    ...
    {
        // printf("%d, %d, %d\n", ss->size1, ss->size2, ss->size3);
        //  printf("min : %d\n", min(target->bottom, src->top, src->bottom)->value);
        //  printf("src top %d  , src bottom %d", src->top->value, src->bottom->value);
